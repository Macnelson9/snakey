...
// An arbitrary cap key (in production this is the GoodDollar root, fed by settle).
...
export function runStoreContract(
  ...
  test(`${name}: create issues a single-use session with a uint32 seed`, async () => {
    const store = await makeStore();
    const s = await store.create({ player: PLAYER, ttlMs: 60_000 });
  ...
  test(`${name}: a session past its TTL is gone`, async () => {
    const store = await makeStore();
    const s = await store.create({ player: PLAYER, ttlMs: 0 });
